...
guidance about what monomer sequences define the autocatalytic set, and a mechanism (\blue{This 
sentence is too vague. We should state our findings more directly.}).\red{A unique aspect of this 
proposal is that it suggests a single mechanism that gives a simple 
rule relating monomer sequences with their autocatalytic ability. Based on the sequence one can 
predict the fold, and from the fold one can determine catalytic properties of the chain. Thus by 
determining only parameters of hydrophobic interaction we can predict catalytic powers of 
$HP$ sequences and their interactions.}. Our work demonstrates that even in random
...
self-sustaining.  We review briefly below other work on those questions.
 ...
 What molecules might have triggered the CTB transition?  Early on, it was recognized by Eigen, 
...
feedback. Various aspects of these systems have been investigated, such as ability to 
reach biologically reasonable lengths (for polymeric systems), maintain exponential growth, 
inheritance and evolvability. And what mechanism of positive feedback gives all the desirable 
...
and cell division was studied. Those works showed that such systems can maintain a 
...
only one type of monomer, which removes evolution, diversity and effects of ``parasitic'' species 
...
transition, where system produces longer chains and maintains itself. The system effectively 
doesn't 
distinguish between polymers of different composition and therefore is the subject to similar 
limitations as in the work of Wu ad Higgs~\cite{Wu2009}.
...
are important for our understanding of autocatalysis. Unfortunately, due to natural complexity of 
life-like systems those studies are focused only on very simple parts of the systems consisting 
only of two or three specifically designed molecules. Thus emergence of autocatalytic sets is not 
...
\sout{The slope of
the Flory plot is governed by $a$, the equilibrium constant for bonding each monomer into
the chain. } 
\blue{I felt this paragraph ended a 
little abruptly. Is there an additional sentence we can add summarizing the point?}
\red{One needs 
...
surface. \red{We propose} this patch can serve as a sticky spot for another HP molecule $B$ and for 
an H monomer 
...
prebiotic chemistry might escape the Flory Problem. \blue{It is not clear from what’s written here 
thus far: is having exposed hydrophobic groups a well documented concept to achieve some rate 
enhancement (in which case we should reference it), or is this an assertion of this manuscript?} 
\red{It is assertion, I mentioned this above.}
...
\subsection{The model of $HP$ polymerization kinetics}
 ...
 \blue{If there is room, I think a simple figure showing the relevant players and rates would be 
very helpful and satisfying to the reader.}\red{I need to think how I can show that.}
...
states 
can `recognize', bind, and react on them.  
\blue{I think it is an interesting point: How much margin of safety do we have here?  In other 
words, what are the relative timescales for chain rearrangements, to binding to a transition state 
and reducing its free energy?  Is it order and orders?  Or is it just barely enough to be 
discernable? } \red{I will have to calculate it, but it can be done, I believe.}
For example, serine proteases utilize a catalytic triad 
...
by forcing $H$ monomers inside will attract another hydrophobes from the solution. \blue{Wouldn't a 
cleft be better than an exposed patch?}
...
serve as catalysts. \blue{I don't know if we make this point clear: Are the rare catalytic folded 
sequences with exposed hydrophobic patches in their native state? Or are we saying they are 
kinetically trapped?}}
  \label{fig:fold-cat}
\end{figure}
...
hyotheses such as the present one. \blue{These points are very important!  I think they can be 
echoed more strongly in the abstract, cover letter, conclusion, etc.}
...
sequences fold to unique structures ($2.3\% $ for lengths up to 25-mers) \blue{do we need to define 
“unique”.  It is an interesting question.} \red{Yes, otherwise model will become too complex for 
any calculation.}.  The fraction of all 
possible HP sequences that have catalytic surfaces (as defined above) is $12.7\%$ of foldable 
sequences, or $0.3\%$ of the whole sequence space. \blue{this is a little unclear to me: Isn't 
there  a minimum length to fold?  earlier we suggest 30-60 monomers are needed.  Why do we say 
anything less than 25mer?} \red{This is a very good point, which it's hard to address, because short 
sequence ``folding'' is a key aspect for our model to work. We basically redefine a ``fold'' from a 
solid classical structure to a semi-molten globule type structure and this should be addressed, 
possibly several times through the paper. Maybe we should call them sequences with unique lowest 
free energy folded state?}  These ratios remain relatively constant with 
