...
surface.
\red{Such exposed cluster of hydrophobes or ``hydrophobic patches'' are fairly common for real 
...
very helpful and satisfying to the reader.}\red{See right before Discussion section}
...
catalyze a reaction. \blue{I think it is an interesting point: How much margin of safety do we have 
here?  In other 
...
discernable? } \red{that requires some literature search and calculations. it might be possible to 
do it.}
...
When folded chain has a patch of at least three exposed nonpolar monomers in its native state, it 
can be a catalyst. 
...
$10^6$ time points in the steady state interval. Black line is a median over 30 simulations }}
  \label{fig:biomass}
\end{figure}
Figure \ref{fig:mainplayers}(a) shows some top sequences as they emerge out of one of our 
simulations in Experiment 3. Sequences are shown as triangles, with the color of triangles 
corresponding to the hydrophobicity of the chains. The folds of some top sequences are shown in 
...
from $50-50,000$, with effect of autocatalysis being visible.
\begin{figure}[h!]
  \centering
...
anything less than 25mer?} \red{ We basically redefine a ``fold'' from a 
